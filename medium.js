...
function isMandarinUser(name, bio, posts) {
  ...
  const contents = _.values(posts).map(
    (item) => item.title + _.get(item, "content.subtitle")
  );
  return Boolean(
    contents.find((item) => {
...
module.exports = {
  ...
  getUserInfo: async (uid) => {
    ...
    return {
      ...
      isWriterProgramEnrolled: _.get(
        json,
        "payload.user.isWriterProgramEnrolled"
      ),
